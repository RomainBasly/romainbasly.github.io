...
After implementing **image optimization in Astro**, the results improved significantly:
...
| **Speed Index - First Visit** | 2.0s                | 1.7s        | 2.3s        | **1.8s**               |
| **Speed Index - Second Visit**| 1.7s                | 1.5s        | 2.0s        | **0.8s**               |
| **Page Weight - First Visit** | 1,791kB             | 347kB       | 1,832kB     | **306kB**              |
| **Page Weight - Second Visit**| **6kB**             | 10kB        | **1,832kB** | **0kB**                |
...
<img 
  src="/Images/blog-posts/2/schema-csr-ssr.png" 
...
Astro differs mainly from Next.js in its default configuration by sending zero JavaScript to the browser unless it's necessary. In Astro, only the parts of your page that require interactivity are enhanced with JavaScript: we call it selective hydration when you indicate which component requires to be hydrated with JS to become interactive. We also call it the "island architecture", since each component works as an island separated to the others and you need to decide which ones need to be hydrated to allow user interactions.
This approach keeps the page lightweight and can improve significantly performance by reducing the JavaScript payload.
Similarly, recent versions of Next.js have introduced the app directory and Server Components, which allow developers to render components on the server without sending JavaScript to the client for those components. This means Next.js can also deliver zero JavaScript to the browser for server-rendered components, aligning with Astro's approach. 
In fact, despite Next.js 13+ offering server components and the ability to serve static content efficiently, we observed that building a simple static site with Next.js resulted in longer build times and larger bundle sizes compared to Astro.
</div>
...
In our tests, **Astro performed exceptionally well**, especially after image optimization. It is no surprise that the framework communicates that it is specifically made for static websites, hence it has been made specifically for that purpose.
...
* Choose Astro for content-focused sites where performance and simplicity are highest priority.
* Consider React when you need flexibility and plan to build highly interactive interfaces or expand to mobile platforms.
* Select Next.js if you require server-side rendering, SEO optimization, and a robust framework that enhances React's capabilities for larger applications.
</div>
...
<ul>
  <li><a href="https://www.webpagetest.org/result/241115_AiDcYZ_ANT/" target="_blank" rel="noopener noreferrer">React</a></li>
  ...
  <li><a href="https://www.webpagetest.org/result/241115_AiDc8C_AQ9/" target="_blank" rel="noopener noreferrer">Astro Optimized Image Test</a></li>
</ul>
</div>
